...
TSMP（Trans Sync Media Protocol）は、VRChat ワールドのランタイムデータをテクスチャストリームで送るオープンソースパッケージです。Encoder がシーンのデータをピクセルに変換し、キャプチャ・配信経路を経由した映像を Decoder が読み取り、対応する受信オブジェクトに適用します。
