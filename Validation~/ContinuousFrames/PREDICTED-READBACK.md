...
The measurements above use the original manually driven coroutine harness, not the automatic `Update` path. A subsequent [completion-retry investigation](COMPLETION-RETRY.md) adds automatic-mode measurement and shows that phase/instrumentation differences materially affect the result. Do not compare those runs directly as evidence of an additional production speedup.
