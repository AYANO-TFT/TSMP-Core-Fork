...
try {
    ...
    if ($Step -in @('TransSync', 'TransSyncVm')) {
    ...
    if ($Step -eq 'Player') {
        ...
        if ($Step -in @('Import', 'Build', 'InitializeSdk', 'Udon', 'ArrayCache', 'UdonArrays', 'EditorEncoding', 'NetworkEdges', 'RpcDelivery', 'Configuration', 'TimelineRegression', 'TransSync')) { $arguments += ' -quit' }
