...
`TSMPDecoder` はヘッダーパスの前に、各スロットが所有する同じサイズの再利用可能な linear `ARGBFloat` RenderTexture へ入力をキャプチャします。ヘッダー・キャリブレーション・payload の全パスは処理終了までこの画像を読みます。元画像が更新・差し替えられても異なる時点の画像が混ざりません。空きスロットはキャプチャ前にサイズを変更できます。無効化すると使用中のスロットを取り消し、空きリソースを解放します。保留中の要求は適用せず完了を待ち、その後リソースを解放・再利用できます。破棄時は所有テクスチャを解放します。キャプチャ失敗時は更新中の元画像へ戻らず処理を拒否します。

受理したデコード試行ごとに画像全体の GPU Blit が1回、**割り当て済みスロットごとに**1ピクセル16バイトのスナップショットメモリが必要です。CPU readback や通信形式のフィールドは追加しません。既定で有効な[制限付き重複実行](../scripting-api/decoder.md#bounded-readback-overlap)は独立した要求状態で最大2フレームを保持し、キャプチャ順に適用します。後の結果が先に完了しても前のフォールバックを待ちます。容量が埋まると新しいキャプチャをスキップし、無制限のキューは蓄積しません。Float32 はサンプリングした入力値に8ビット・half-float の量子化を追加しないための選択です。所有権とリソース条件は [decoder API](../scripting-api/decoder.md#input-snapshot) を参照してください。入力済みの壊れた画像を修復したり、変数・RPC の適用をトランザクションにしたりする機能ではありません。
