...
以下は初回・フォールバック経路です。既定の予測経路は両方のバイトパスを先に実行して1回 readback し、現在のヘッダーを検証してから payload を採用します。メタデータまたは正確な payload 長が変われば、同じスナップショットで通常の payload デコードを行い、CRC 不一致ではフレームを破棄します。カスタムコーデックと送信形式は変更しません。比較フィールド・バッファ構造・所有権は[予測 readback](../scripting-api/decoder.md#predicted-readback)を参照してください。
...
ApplyDecodeOptions
  ...
  -> 個別 readback、またはヘッダー/payload をまとめて予測 readback 1回
```
...
`TSMPDecoder` はヘッダーパスの前に、各スロットが所有する同じサイズの入力形式を維持する再利用可能な RenderTexture へ入力をキャプチャします。ヘッダー・キャリブレーション・payload の全パスは処理終了までこの画像を読みます。元画像が更新・差し替えられても異なる時点の画像が混ざりません。空きスロットはキャプチャ前にサイズを変更できます。無効化すると使用中のスロットを取り消し、空きリソースを解放します。保留中の要求は適用せず完了を待ち、その後リソースを解放・再利用できます。破棄時は所有テクスチャを解放します。キャプチャ失敗時は更新中の元画像へ戻らず処理を拒否します。

受理したデコード試行ごとに画像全体の GPU Blit が1回、**割り当て済みスロットごとに**形式に応じて1ピクセル4・8・16バイトのスナップショットメモリが必要です。CPU readback や通信形式のフィールドは追加しません。既定で有効な[制限付き重複実行](../scripting-api/decoder.md#bounded-readback-overlap)は独立した要求状態で最大2フレームを保持し、キャプチャ順に適用します。後の結果が先に完了しても前のフォールバックを待ちます。容量が埋まると新しいキャプチャをスキップし、無制限のキューは蓄積しません。判別可能な8-bit・half-float 入力は保存精度と色解釈を維持し、Float32 と不明な入力は Float32 を維持します。Udon は RenderTexture 以外の入力に保守的に Float32 を使います。所有権とリソース条件は [decoder API](../scripting-api/decoder.md#input-snapshot) を参照してください。入力済みの壊れた画像を修復したり、変数・RPC の適用をトランザクションにしたりする機能ではありません。
