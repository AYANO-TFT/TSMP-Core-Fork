...
LUT の割り当ては再利用しますが、有効な各パスで内容を更新します。`TSMPDecoder` は固定した入力スナップショットを準備・バイトシェーダーの両方に渡します。LUT 準備は元画像を再キャプチャせず、前のデコード処理の値も再利用しません。コーデックを直接呼ぶ実装では入力画像を自分で固定してください。
