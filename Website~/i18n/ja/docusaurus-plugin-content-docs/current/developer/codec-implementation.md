...
この例には Core **0.3.0-beta.2** で追加された準備 API が必要です。Core 0.2.0 と 0.3.0-beta.1 にはこの API がありません。依存コーデックより先に対応する Core をインストールし、コーデックを公開する前に実際の配布パッケージの組み合わせを検証してください。
...
キャリブレーションは、受信映像内の既知の基準シンボルを測定し、ペイロードのサンプルと比較する処理です。出力バイトごとに同じ基準ブロックを読むと処理が重複します。LUT（ルックアップテーブル）を使うと、その計算を一度の GPU 準備パスへ移せます。

LUT は計算する場所だけを変えます。パケット形式やシンボルの判定アルゴリズムは変えません。追加パスの費用が上回るコーデックは、従来のデコード経路を維持してください。

### 呼び出し順序

Decoder は**ヘッダーまたはペイロードの各バイトパスで**次の順に処理します。

1. コーデックを選び、`ApplyDecodeOptions()` でバイトマテリアルとレイアウトを決めます。
2. `_MainTex`、入力サイズ、ブロック/サンプルサイズ、`_StartBlock`、`_ByteCount`、バイト出力サイズ、`_FlipY` を設定します。
3. `PrepareDecode(source, material)` を呼びます。コーデックは今回の設定でキャリブレーションを準備できます。
4. 直後に同じ入力 Texture 参照をバイトマテリアルで Blit します。
5. LUT ではなく、バイト出力テクスチャから readback を要求します。

`ApplyDecodeOptions()` 内で準備しないでください。その時点ではパスの全プロパティがまだ設定されていません。`Start()`、コーデック変更時、フレームごとに一度だけの準備も不十分です。ヘッダーとペイロードのパスは設定と実行時点が異なる場合があります。

`TSMPDecoder` はヘッダーパスの前に入力を自身のスナップショットへコピーし、そのデコード処理のすべての準備・バイトパスへ渡します。渡されたテクスチャを変更・解放しないでください。映像側が元画像を更新しても、ヘッダーと payload は同じキャプチャ画像を読みます。フック自体は画像をコピーしないため、`TSMPDecoder` を使わずにコーデックを呼ぶ実装では入力を自分で固定してください。

### Luma4 の最小フック

Luma4 ハンドラーは次のオーバーライドを使います。既存のコーデッククラス内で、エンコードメソッドと `ApplyDecodeOptions()` に加えて実装します。
...
Luma4 の基準シンボルは16個なので、テーブルは16×1です。有効サンプルサイズが1の場合、計測環境では準備費用が削減量を上回ったため従来経路を使います。これは Luma4 の方針であり、Core が他のコーデックへ強制する条件ではありません。独自コーデックでは準備費用も含めて計測し、条件を決めてください。

コーデックのプレハブで、継承した `calibrationMaterial` フィールドに Luma4 準備シェーダーのマテリアルを設定します。このフィールドは通常の Inspector では非表示です。パッケージのエディターツールまたは Inspector の Debug モードで設定してください。利用者には、追加操作が不要な設定済み catalog/プレハブを提供します。

バイトシェーダーにも `_CalibrationLut`、`TSMP_CALIBRATION_LUT` の local variant、従来サンプリング経路が必要です。[シェーダーガイド](./codec-shaders.md)に実装を示します。C# フックだけを追加してもシェーダーは LUT を使いません。

### 所有権とフォールバック

- コーデックの基底クラスが生成した LUT RenderTexture を所有します。割り当ては再利用し、有効な各パスで内容を描き直し、無効化・破棄時に解放して破棄します。
- `calibrationMaterial` とバイトマテリアルは外部から渡す参照です。フックは生成しませんが、プロパティを変更します。Setup はコントローラーごとのマテリアルコピーを用意します。独自の組み込みでも共有マテリアルの競合を防いでください。
- 有効条件を調べる前に、必ず `base.PrepareDecode(source, material)` を呼びます。同じマテリアルが従来経路に戻る場合も、前の LUT キーワードを解除する必要があります。
- `OnDisable()` または `OnDestroy()` をオーバーライドしたら対応する基底実装も呼んでください。パッケージのマテリアルアセットや外部の入力/出力テクスチャを破棄しないでください。
- 入力、バイトマテリアル、準備マテリアルがない場合、不正な LUT 幅、空のバイト出力では準備を省略します。Native は準備シェーダーと ARGBFloat の対応状況も確認します。両経路で実際のフォーマットとテクスチャ生成結果を確認するため、従来のシェーダー variant を残してください。
- 1行の linear `ARGBFloat`（各チャネル32-bit float）、Point フィルター、mipmap なしを使います。Half・8-bit・sRGB 保存への変更は丸めにより判定境界のシンボルを変える可能性があります。
- 準備パスが複数なら順序を定め、各出力を完全に上書きします。書き込み中の LUT を同時に読まないでください。

宣言、ガード条件、ライフサイクルは [TSMPCodec Scripting API](../scripting-api/codec.md#runtime-decode-preparation) を参照してください。リソース不足、サンプル変更、無効化と再有効化、複数インスタンス、Player の両 variant をバイト単位で比較検証してください。
