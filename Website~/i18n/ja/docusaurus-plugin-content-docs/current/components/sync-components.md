...
Sync Rigidbody は、Rigidbody がある場合に速度と角速度を送受信し、受信した位置・回転も Rigidbody に適用します。受信側で無効にするとローカルの速度と角速度を維持し、Transform の位置・回転・スケールは引き続き同期します。Discrete と Continuous の両方に適用されます。無効化時に保留中の速度補間目標も破棄するため、再有効化後の速度更新は次の受信パケットから再開します。質量・重力・Kinematic はローカル設定で、このコンポーネントでは送信しません。
