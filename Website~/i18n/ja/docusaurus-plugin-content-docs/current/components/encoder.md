...
自動変数は [TransSync 送信スケジューリング](../scripting-api/transsync.md) に従います。変化のない値は既定で 1 秒ごとに再送するため、静止したシーンで再送の間にフレーム番号が止まるのは正常です。他のコンポーネントの再送間隔を変えずに、特定のコンポーネントだけを頻繁に送るには、そのコンポーネントで [Send Mode: Always](./network-behaviour.md#send-mode) を選びます。最小間隔と容量制限は引き続き適用されます。
...
自動変数の合計が容量を超えた場合は高優先度のフィールドから書き込み、残りを後の試行に回します。**Deferred Variables** と頻度制限付きの警告で確認できます。フィールド単体がペイロード全体より大きい場合、サイズを減らすか出力容量を増やす必要があります。不正な値や大きすぎる RPC・手動メッセージは、引き続きエンコード失敗になる場合があります。
