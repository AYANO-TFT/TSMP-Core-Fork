...
LUT 할당은 재사용하지만 활성 패스마다 내용을 갱신합니다. `TSMPDecoder`는 준비·바이트 셰이더 모두에 고정된 입력 스냅샷을 제공합니다. LUT 준비는 원본을 다시 캡처하지 않으며, 이전 디코딩 작업의 캘리브레이션 값을 재사용하지도 않습니다. 코덱을 직접 호출하는 코드는 입력 이미지를 직접 고정해야 합니다.
