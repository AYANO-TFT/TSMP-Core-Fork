...
`PrepareDecode(Texture source, Material material)` is called immediately before each header or payload byte pass, after the decoder has assigned the source dimensions, sample size, byte count and layout. It receives the same Texture reference as the following byte blit, not a frozen copy of its pixels. Header and payload passes are separate: this API does not guarantee they sample the same video frame. Override it for optional GPU preparation, and call `base.PrepareDecode(source, material)` first to clear the previous LUT keyword. Existing codecs need no override.
